...
void AddBUFFERgate::ReadActionParameters()
{
	...
	do {
		...
		if (Cy<(UI.ToolBarHeight + UI.BUFFER_Width / 2) || Cy >(UI.height - UI.StatusBarHeight - UI.BUFFER_Width / 2)) {
		}
	} while (Cy<(UI.ToolBarHeight + UI.BUFFER_Width / 2) || Cy >(UI.height - UI.StatusBarHeight - UI.BUFFER_Width / 2));
...
void AddBUFFERgate::Execute()
{
	...
	GInfo.y1 = Cy - Wdth / 2;
	GInfo.y2 = Cy + Wdth / 2;
	pA = new BUFFER(GInfo, BUFFER_FANOUT);
